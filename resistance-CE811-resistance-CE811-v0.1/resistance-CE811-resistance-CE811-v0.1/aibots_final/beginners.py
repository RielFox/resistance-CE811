...
class Paranoid(Bot):
    ...
    def vote(self, team):
        ...
        #vote up all missions that bot is part of
        if self in self.team:
            return true

        #vote up all missions if it is on the last team selection attempt
        if self.tries == 5:
            return true
        else:
            return false
    ...
    def onMissionComplete(self,sabotaged):
        ...
        self.sabotages = sabotaged

        if self.sabotages > 0:
            print('This mission has been sabotaged!')
            self.announce()
    ...
    def announce(self):
        ...
        for player in self.game.team:
            dict[player] = 1/len(self.game.team)
    ...
    #If this is the first mission, do not sabotage
        if self.turn == 1:
            return False
        elif len(self.team) <= 2 : #Never sabotage if only 2 people on a mission
            return False
        else: return True
